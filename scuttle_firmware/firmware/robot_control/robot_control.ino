...
// --- TUNING VALUES (From Code A) ---
// Note: These tune RPM. We convert ROS Rad/s to RPM before PID.
double Kp_L = 2.5, Ki_L = 0.05, Kd_L = 0.02;
double Kp_R = 2.55, Ki_R = 0.06, Kd_R = 0.01;

// --- UNIT CONVERSION ---
const float RPM_TO_RADS = 0.104719755;
const float RADS_TO_RPM = 9.54929658;

// --- SERIAL PARSING STATE (From Code B) ---
...
void setup() {
  // ROS usually prefers higher baud rates
  Serial.begin(115200);
  ...
  // Ensure direction matches your physical wiring
...
void loop() {
  // -------------------------------------------------
  // 1. SERIAL COMMAND PARSER (From Code B)
  // -------------------------------------------------
  if (Serial.available()) {
  ...
  if(now - prevTime >= 50) { 
    ...
    double rpmL = getRPM(dt, &encL, &prevAngL, &prevTotL, &rotL, bufL);
    double rpmR = getRPM(dt, &encR, &prevAngR, &prevTotR, &rotR, bufR);

    // --- B. CONVERT TARGETS (Rad/s -> RPM) ---
    // We convert the ROS command to RPM to use your existing PID tunings
    ...
    // --- D. APPLY TO MOTORS ---
    // If target is 0, hard stop to prevent drift
    ...
    // Format: "rp3.14,ln2.50,"
    Serial.print("r");
    Serial.print(r_sign);
    Serial.print(abs(meas_rads_R));
    Serial.print(",l");
    Serial.print(l_sign);
    Serial.print(abs(meas_rads_L));
    Serial.println(",");
  }
}
...
// HELPERS (From Code A)
// -------------------------------------------------
double getRPM(double dt, AMS_AS5048B *enc, double *prevAng, double *prevTot, long *rot, double *buf) {
  ...
  *prevTot = tot;
  ...
  buf[idx] = raw;
  // idx is global (shared), not ideal but works for synchronous calls
  // Better to reset idx logic if you expand buffers, but works here
  double avg = 0;
