#include "scuttle_planning/a_star_euclidean_planner.hpp"
#include "pluginlib/class_list_macros.hpp"
#include <limits>

PLUGINLIB_EXPORT_CLASS(scuttle_planning::AStarEuclideanPlanner, nav2_core::GlobalPlanner)

namespace scuttle_planning
{

void AStarEuclideanPlanner::configure(
...
void AStarEuclideanPlanner::cleanup() {}
void AStarEuclideanPlanner::activate() {}
void AStarEuclideanPlanner::deactivate() {}

nav_msgs::msg::Path AStarEuclideanPlanner::createPlan(
  ...
  std::function<bool()> cancel_checker) // ERROR FIX: Removed '&'
{
  ...
  float h = std::hypot((int)mx_start - (int)mx_goal, (int)my_start - (int)my_goal);
  pq.push({start_idx, 0.0f + h});

  // 8-Connectivity (Cardinals + Diagonals)
  const int dx[8] = {0, 0, 1, -1, 1, 1, -1, -1};
  const int dy[8] = {1, -1, 0, 0, 1, -1, 1, -1};
  // Move costs: 1.0 for straight, 1.414 (sqrt(2)) for diagonal
  const float step_costs[8] = {1.0, 1.0, 1.0, 1.0, 1.414, 1.414, 1.414, 1.414};
  ...
  while (!pq.empty()) {
    ...
    int ux_int = u % size_x;
    int uy_int = u / size_x;
    float current_h = std::hypot(ux_int - (int)mx_goal, uy_int - (int)my_goal);
    ...
    for (int i = 0; i < 8; i++) {
      ...
      float weight = step_costs[i] * (1.0 + (cost / 255.0)); 
      ...
      if (tentative_g < g_cost[costmap_->getIndex(nx, ny)]) {
        ...
        parent[v] = u;
        float h_neighbor = std::hypot(nx - (int)mx_goal, ny - (int)my_goal);
